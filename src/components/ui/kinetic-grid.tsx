...
export default function KineticGrid({
  ...
  const draw = useCallback(
    (now: number) => {
      ...
      const theme = {
        ...
        brand: {
          bg: "#0A0A0A",
          lineActive: { r: 197, g: 255, b: 0, a: 0.7 },
          nodeActive: { r: 197, g: 255, b: 0, a: 0.9 },
          glow: "197,255,0",
          ripple: "197,255,0",
        },
      }[globalColor ?? "default"];
  ...
  useEffect(() => {
    ...
    const isAbsolute = canvasPosition === "absolute";

    const setSize = () => {
      const w = isAbsolute ? container.offsetWidth : window.innerWidth;
      const h = isAbsolute ? container.offsetHeight : window.innerHeight;
      canvas.width = w;
      canvas.height = h;
      sizeRef.current = { w, h };
    };
    ...
    const onMouseMove = (e: MouseEvent) => {
      targetMouseRef.current = getCanvasCoords(e.clientX, e.clientY);
    };
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (touch) {
        targetMouseRef.current = getCanvasCoords(touch.clientX, touch.clientY);
      }
    };
    ...
    >
      <canvas
        ...
        className={cn(
          ...
          canvasPosition === "absolute" ? "absolute" : "fixed",
        )}
      />
